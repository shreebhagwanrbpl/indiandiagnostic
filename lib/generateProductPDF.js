...
export async function generateProductPDF(product, activeImageUrl = "", city = "India") {
    ...
    doc.setFontSize(22);
    doc.text("RAJ BIOSIS", 14, 15);

    doc.setTextColor(...brightYellow);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(9);
    doc.text("MEDICAL & CLINICAL DIAGNOSTIC SOLUTIONS", 14, 22);
    ...
    const specs = [
        { label: "Brand", value: product.brand || "Raj Biosis / Partner" },
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(9);
    doc.text("WHY CHOOSE RAJ BIOSIS", 14 + boxWidth + 10, currentY + 5.5);
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8.5);
    doc.text("Raj Biosis - Diagnostic Instruments & Healthcare Solutions", 14, footerY + 7);
