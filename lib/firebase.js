...
async function safeFetchFirestore(url, options = {}, timeoutMs = 6000, maxRetries = 1) {
  let lastErr;
...
export async function getDoc(docRef) {
  ...
  try {
    ...
    const res = await safeFetchFirestore(url, fetchOptions, 10000, 1);
    ...
    console.warn(`Firestore getDoc skipped for ${docRef.path}: ${err?.message || err}`);
    return {
      exists: () => false,
      id: docRef.id,
      data: () => undefined,
...
export async function getDocs(colRefOrQuery) {
  ...
  try {
    do {
      ...
      const res = await safeFetchFirestore(url, fetchOptions, 20000, 1);
    ...
    console.warn(`Firestore getDocs skipped for ${colRefOrQuery.path}: ${err?.message || err}`);
    return {
      empty: true,
      docs: [],
      size: 0,
...
export async function deleteDoc(docRef) {
  ...
  try {
    ...
    console.warn(`Firestore deleteDoc error for ${docRef.path}:`, err);
  }
}
