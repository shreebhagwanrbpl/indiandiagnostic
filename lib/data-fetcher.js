 ...
 * 2. websiteIds contains WEBSITE_ID (indiandiagnostic) or "all"
 * Note: indiandiagnosticscom is a separate website and is strictly isolated.
 */
 ...
 * Reads from:
 * 1. companies/[companyId]/categories + nested subcategories for the primary company (rajbiosis)
 * 2. companies/[companyId]/products for the primary company
...
export async function getAggregatedCatalogServer(websiteId = WEBSITE_ID, primaryCompanyId = COMPANY_ID) {
  ...
  // Unique list of companies to scan (only primary company rajbiosis)
  const companiesToScan = [primaryCompanyId || COMPANY_ID];
...
const CACHE_TTL_MS = 15000; // 15s server memory cache for instant slug lookups, overridden by forceRefresh
...
export async function fetchFullCatalog(forceRefresh = false) {
  ...
  if (typeof window !== "undefined") {
    try {
      const queryParam = forceRefresh ? `?refresh=true&t=${Date.now()}` : "";
      const res = await fetch(`/api/catalog${queryParam}`);
      if (!res.ok) {
